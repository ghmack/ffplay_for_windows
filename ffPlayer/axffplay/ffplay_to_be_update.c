...
{
	if(!texture) 
		texture = SDL_CreateTexture(render, SDL_PIXELFORMAT_YV12,SDL_TEXTUREACCESS_STREAMING,overlay->w,overlay->h);
...
static int video_open(VideoState *is, int force_set_video_mode, VideoPicture *vp)
{
	...
	SDL_GetWindowSize(screen, &pre_w,&pre_h);
	//if (screen && is->width == screen->w && screen->w == w
	//	&& is->height== screen->h && screen->h == h && !force_set_video_mode)
	//	return 0;
   // screen = SDL_SetVideoMode(w, h, 0, flags);

	if (screen && is->width == pre_w && pre_h == w
   ...
   // SDL_WM_SetCaption(window_title, window_title);
	...
	SDL_GetWindowSize(screen, &is->width,&is->height);
...
static int audio_open(void *opaque, int64_t wanted_channel_layout, int wanted_nb_channels, int wanted_sample_rate, struct AudioParams *audio_hw_params)
{
	...
	//while (SDL_OpenAudioDevice(NULL,0,&wanted_spec, &spec,SDL_AUDIO_ALLOW_ANY_CHANGE) < 0) {
...
int start_running(int argc, char **argv,HWND hwnd)
{
    int flags;
    VideoState *is;
